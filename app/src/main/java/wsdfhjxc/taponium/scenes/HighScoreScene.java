...
public class HighScoreScene extends Scene {
    ...
    private Bitmap highScorePanelBitmap; // 하이스코어 패널 비트맵 객체
    private Rect highScorePanelRect; // 하이스코어 패널을 둘러싼 사각형 객체
    private Flex highScorePanelFlex; // 하이스코어 패널 flex 객체
    ...
    public void load() {
        ...
        highScorePanelBitmap = resourceKeeper.getBitmap("highscore_panel"); // 타이틀 패널 이미지 로드
        // 타이틀 패널 이미지를 둘러싼 사각형을 너비와 높이만큼 설정
        highScorePanelRect = new Rect(0, 0, highScorePanelBitmap.getWidth(), highScorePanelBitmap.getHeight());
        // 타이틀 패널 이미지를 스마트폰 크기에 따라 비트맵의 실제 크기와 위치를 조정
        highScorePanelFlex = new Flex(new PointF(0f, 0f), false,
    ...
    public void handleRender(Canvas canvas, Paint paint, double alpha) {
        canvas.drawBitmap(highScorePanelBitmap, highScorePanelRect, highScorePanelFlex.getRect(), paint); // 하이스코어 패널을 그린다.
