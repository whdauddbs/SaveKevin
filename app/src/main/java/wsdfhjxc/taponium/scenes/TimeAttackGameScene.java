...
public class TimeAttackGameScene extends Scene {
    private TimeCounter timeCounter; // 시간 카운터 객체
    private TimeCounterRenderer timeCounterRenderer; // 시간 카운터 렌더러 객체
    ...
    private Bitmap watchBitmap; // 일시정지 버튼(게임 내) 비트맵 객체
    private Rect watchRect; // 일시정지 버튼(게임 내) 비트맵을 둘러싼 사각형 객체
    private Flex watchFlex; // 일시정지 버튼(게임 내) 비트맵 flex 객체

    private  boolean isClicked = false;

    public TimeAttackGameScene(SceneKeeper sceneKeeper, ResourceKeeper resourceKeeper, FlexConfig flexConfig) {
        super(sceneKeeper, resourceKeeper, flexConfig, 7, 1); // Scene 클래스 생성자
    }

    @Override
    public void load() {

        timeCounter = new TimeCounter(); // 시간 카운터 객체 생성
        timeCounterRenderer = new TimeCounterRenderer(timeCounter, resourceKeeper, flexConfig); // 시간 카운터 렌더러 객체 생성
        timeCounter.update(); // 게임 시작하자마 시간이 흐르도록 설정

        watchBitmap = resourceKeeper.getBitmap("watch"); // 시계 아이콘 객체 이미지 객체 생성
        // 시계아이콘 이미지를 둘러싼 사각형을 너비와 높이만큼 설정
        watchRect = new Rect(0, 0, watchBitmap.getWidth(), watchBitmap.getHeight());
        // 뒤로가기 버튼을 스마트폰 크기에 따라 비트맵의 실제 크기와 위치를 조정
        watchFlex = new Flex(new PointF(0.1f, 0.2f), false,
                new PointF(watchBitmap.getWidth(), watchBitmap.getHeight()), true,
                new Point(-watchBitmap.getWidth()/2, -watchBitmap.getHeight()+40),
                flexConfig);
    ...
    public void handleUpdate(double deltaTime) { // handleUpdate함수(터치 입력) 오버라이딩
        ...
        if (timeCounter.isNegative()) { // 제한 시간이 모두 소요 되면 게임 오버
    ...
    public void handleRender(Canvas canvas, Paint paint, double alpha) {
        ...
        timeCounterRenderer.render(canvas, paint);
        ...
        canvas.drawBitmap(watchBitmap, watchRect, watchFlex.getRect(), paint); // 뒤로가기 버튼(게임 내)을 그린다.
    }
}
